...
/**
 * Renders a full proposal (all sections) for a single edital. Extracted from
 * the former home page so it can be reused by every `/edital/[slug]` route.
 * `showVersioning` gates the version/diff subsystem, which is tied to the
 * legacy AgriFam commit history.
 */
export default function ProposalView({
  ...
  docs?: Array<{ label: string; file: string; tag?: string }>
}) {
    ...
    <LoaderProvider>
      <VersionProvider>
        <div className="flex min-h-screen" style={{ background: 'var(--bg)' }}>
          <PageLoader projectName={meta.projectName} call={meta.call} para={para} />
          <Sidebar
            ...
            <Reveal>
              <DiffSection sectionKey="s7">
                <section id="s7" className="scroll-mt-4">
                  <ParceriasSection {...s7} deadline={meta.deadline} />
                </section>
              </DiffSection>
            </Reveal>
            <Reveal>
